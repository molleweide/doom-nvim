        ...
        {
          {
            "L",
            function()
              -- NOTE: This needs to be handled with treesitter, due
              -- to observer problem - we want to control whether or
              -- not to reload but it should not be affected by
              -- reloading itself. Therefore this is a special variable
              -- that has to be handled in code explicitly and
              -- then set it with treesitter, and finally reload the
              -- reloader module with the module_manager.api,
              -- TODO: Toggle value with treesitter
              -- TODO: Should the value be put on a hidden variable maybe.
              --       >> reloader._reload_on_save.
              -- TODO: create a function called `reload_single_module(opts{target = "component|all" (default to all..)})` that
              -- ensures that the settings table is reloaded
