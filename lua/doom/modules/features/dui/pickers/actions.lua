    ...
    if #selections > maxheight then
        a.nvim_buf_set_extmark(buf, a.nvim_create_namespace(""), 0, 0, {
            virt_text = {
                {
                    string.format(
                        "Selections exceed window height: %s/%s shown ",
                        maxheight,
                        #selections
                    ),
                    "Comment",
...
dui_picker_actions.rename = function(prompt_bufnr)
    ...
    if not vim.tbl_isempty(selections) then
        ...
        if not entry then
            fb_utils.notify(
                "action.rename",
                { msg = "No selection to be renamed!", level = "WARN" }
            )
            return
        end
        ...
        if old_path.filename == parent_dir.filename then
            fb_utils.notify(
                "action.rename",
                { msg = "Please select a valid file or folder!", level = "WARN", quiet = quiet }
            )
            return
        end
        get_input(
            { prompt = "Rename: ", default = old_path:absolute(), completion = "file" },
            function(file)
                vim.cmd([[ redraw ]]) -- redraw to clear out vim.ui.prompt to avoid hit-enter prompt
                if file == "" or file == nil then
                    fb_utils.notify(
                        "action.rename",
                        { msg = "Renaming aborted!", level = "WARN", quiet = quiet }
                    )
                    return
                end
...
dui_picker_actions.move = function(prompt_bufnr)
    ...
    if vim.tbl_isempty(selections) then
        fb_utils.notify(
            "actions.move",
            { msg = "No selection to be moved!", level = "WARN", quiet = finder.quiet }
        )
        return
    end
...
dui_picker_actions.copy = function(prompt_bufnr)
    ...
    if vim.tbl_isempty(selections) then
        fb_utils.notify(
            "actions.copy",
            { msg = "No selection to be copied!", level = "WARN", quiet = finder.quiet }
        )
        return
    end
        ...
        while index <= #selections do
            ...
            if destination:exists() then
                ...
                if is_dir and absolute == destination:parent():absolute() then
                    local message =
                        string.format("Copying folder into itself not (yet) supported", name)
                    fb_utils.notify(
                        "actions.copy",
                ...
                elseif is_dir and vim.tbl_contains(parents, absolute) then
                    local message =
                        string.format("Copying a parent folder into path not supported", name)
                    fb_utils.notify(
                        "actions.copy",
                        { msg = message, level = "INFO", quiet = finder.quiet }
                    )
...
dui_picker_actions.remove = function(prompt_bufnr)
    ...
    if vim.tbl_isempty(selections) then
        fb_utils.notify(
            "actions.remove",
            { msg = "No selection to be removed!", level = "WARN", quiet = quiet }
        )
        return
    end
    ...
    for _, sel in ipairs(selections) do
        if sel:is_dir() then
            ...
            if msg then
                fb_utils.notify(
                    "actions.remove",
                    { msg = msg .. " Prematurely aborting.", level = "WARN", quiet = quiet }
                )
                return
            end
        end
    ...
    get_confirmation({ prompt = "Remove selection? (" .. #files .. " items)" }, function(confirmed)
        ...
        if confirmed then
            ...
            fb_utils.notify(
                "actions.remove",
                { msg = "Removing selections aborted!", level = "INFO", quiet = quiet }
            )
        end
    end)
end
...
dui_picker_actions.open = function(prompt_bufnr)
    ...
    local cmd = vim.fn.has("win32") == 1 and "start"
...
dui_picker_actions.goto_parent_dir = function(prompt_bufnr, bypass)
    ...
    if not bypass then
        if vim.loop.cwd() == finder.path then
            fb_utils.notify("action.goto_parent_dir", {
                msg = "You cannot bypass the current working directory!",
                level = "WARN",
                quiet = finder.quiet,
            })
            return
...
dui_picker_actions.goto_home_dir = function(prompt_bufnr)
    ...
    -- this is where we access the finder to get
    finder.path = vim.loop.os_homedir()

    -- this is redrawn based on the current picker.
    fb_utils.redraw_border_title(current_picker)
...
dui_picker_actions.switch_picker = function(prompt_bufnr, opts)
    ...
    -- if a previewer is not provided, then we remove the previous previewer...
    -- ^ Ie. reset anything that we dont want.
...
dui_picker_actions.toggle_browser = function(prompt_bufnr, opts)
    ...
    current_picker:refresh(
        finder,
        { reset_prompt = opts.reset_prompt, multi = current_picker._multi }
    )
end
