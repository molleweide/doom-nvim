...
--
local doom_picker_utils = {}

doom_picker_utils.make_title = function(t)
    return (":: DOOM | %s ::"):format(t)
end
...
--         new_title =
--             truncate(new_title ~= os_sep and new_title .. os_sep or new_title, width, nil, -1)
...
--         local path, _ =
--             Path:new(finder.path):make_relative(finder.cwd):gsub(vim.fn.expand("~"), "~")
...
--         fb_utils.notify(
--             "to_absolute_path",
--             { msg = string.format("Given path '%s' doesn't exist", path), level = "WARN" }
--         )
--         return nil
