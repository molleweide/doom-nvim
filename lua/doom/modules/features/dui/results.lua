...
result_nodes.modules = function()
  local modules_component = {
    ...
    entry_template = function(_, _, module)
      ...
      -- This attaches a list of mappings to each entry.
      -- This means that the mappings table is duplicated for each
      -- entry, which is very slow.
      -- >>>
      -- TEST: Move mappings per category to a look up table and then
      -- check the category of the selected entry and then get the action
      -- from the category and then run if everything is correct.
