To find the topmost `init.lua` files in the `modules` directory in Neovim, while ignoring deeper `init.lua` files within subdirectories, you can use a Lua script with the following logic:
...
2. **Avoid Deeper Traversals**:
   - If an `init.lua` is found in a directory, exclude subdirectories of that directory from further checks.
3. **Results**:
   - Gather paths of `init.lua` files that are the topmost in their respective branches.
...
- `vim.tbl_filter`: Filters out paths of subdirectories if `init.lua` exists in a parent.

This approach ensures you only collect top-level `init.lua` files without being affected by deeper files.
