...
-- TODO: Replace all print statements with vim.nofity()
...
functions.set_indent = function()
  ...
  print(string.format("indent=%d %s", indent, vim.opt.expandtab:get() and "spaces" or "tabs"))
end
...
functions.change_number = function()
  ...
  print(
    ...
    utils.bool2str(vim.opt.number:get()),
    utils.bool2str(vim.opt.relativenumber:get())
  )
end

-- Toggle spell.
functions.toggle_spell = function()
  vim.notify("Toggle spelling")
  vim.opt.spell = not vim.opt.spell:get()
  print(string.format("spell=%s", utils.bool2str(vim.opt.spell:get())))
end
...
functions.toggle_wrap = function()
  ...
  -- maybe break indent should go into its own func.
  vim.opt.breakindent = not vim.opt.breakindent:get()

  vim.notify("Toggle wrap (prev) -> (new) ")
end
